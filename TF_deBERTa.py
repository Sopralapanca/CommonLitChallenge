...
for fold in range(0, config['folds']):
    ...
    train_data_={'x': (train_data['input_ids'].values.tolist(),
                       train_data['attention_mask'].values.tolist()),
                 'y':  train_data['y'].values.tolist()}
    val_data_  ={'x': (val_data['input_ids'].values.tolist(),
                       val_data['attention_mask'].values.tolist()),
                 'y':  val_data['y'].values.tolist()}
    model = build_model()
    model.fit(((train_data['input_ids'].values.tolist(),
                  train_data['attention_mask'].values.tolist(),
                 ),
                  train_data['y'].values.tolist()),
